...
class UserModelCase(unittest.TestCase):
    ...
    def test_density(self):
        ...
        pa.density.bins_plot(dens_s, target_dens=0.146, is_mean=True, pore_id="shape_00")
        ...
        plt.figure()
        pa.density.bins_plot(dens_no_remove, pore_id="shape_00")
        plt.savefig("output/density_no_remove.pdf", format="pdf", dpi=1000)
        # plt.show()

        plt.figure()
        pa.density.bins_plot(dens_slit, is_mean=True, pore_id="shape_00")
        plt.savefig("output/density_slit.pdf", format="pdf", dpi=1000)
        # plt.show()

        plt.figure()
        pa.density.bins_plot(dens_s, intent="in", pore_id="shape_00")
        pa.density.bins_plot(dens_p, intent="in", pore_id="shape_00")
        plt.legend(["Single core", "Parallel"])
        plt.savefig("output/density_in.pdf", format="pdf", dpi=1000)
        # plt.show()

        plt.figure()
        pa.density.bins_plot(dens_s, intent="ex", pore_id="shape_00")
        pa.density.bins_plot(dens_p, intent="ex", pore_id="shape_00")
        plt.legend(["Single core", "Parallel"])
        plt.savefig("output/density_ex.pdf", format="pdf", dpi=1000)
        # plt.show()

        plt.figure()
        pa.density.bins_plot(dens_box, intent="ex", pore_id="shape_00")
    ...
    def test_angle(self):
        ...
        plt.figure()
        mean_s = pa.angle.bins_plot("output/angle_cyl.obj", "output/dens_cyl_s.obj", is_mean=True)
        plt.savefig("output/angle.pdf", format="pdf", dpi=1000)

        plt.figure()
        pa.angle.bins_plot("output/angle_cyl.obj", "output/dens_cyl_s.obj", intent="in", is_norm=True)
        pa.angle.bins_plot("output/angle_cyl.obj", "output/dens_cyl_s.obj", intent="ex", is_norm=True)
        plt.legend(["Interior", "Exterior"])
    ...
    def test_diffusion_bin(self):
        ...
        plt.figure()
        mean_s = pa.diffusion.mean(pa.diffusion.bins("output/diff_cyl_s.obj"), pa.density.bins("output/dens_cyl_s.obj"))
        ...
        self.assertEqual(round(mean_s["shape_00"], 2), 1.13)
        self.assertEqual(round(mean_p["shape_00"], 2), 1.13)
    ...
    def test_diffusion_mc_mc(self):
        ...
        model._len_step = [10]
        # Do the MC alogirthm
        pa.MC().run(model,"output/diff_test_mc_spline.obj", nmc_eq=100, nmc=2000, is_print=True, is_parallel=False)
    ...
    def test_parallel_sample(self):
        ...
        trans_s = data["data"]
        data = pa.utils.load("output/diff_mc_cyl_p.obj")
        trans_p = data["data"]
        data = pa.utils.load("data/check_output_sample.obj")
